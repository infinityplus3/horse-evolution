            ...
            <div style={{ display: "flex", flexDirection: "column", flex:1 }}> 
                ...
                <div style={{float: 'right', border: "5px solid rgba(149, 165, 166, .7)", backgroundColor: 'rgba(149, 165, 166, .5)', borderRadius: '15px', marginBottom: '20px', marginTop: '78px', marginRight: '20px'}}>
                    ...
                    >
                        ...
                        <XAxis dataKey="name" tick={{fontSize: 5, fill:'black'}} fill="black" interval={0} angle={20} textAnchor="beginning"/>
                        <YAxis tick={{fontSize: 15, fill:'black'}} fill="black" domain={[0, 40]}/>
                        <Bar dataKey="da" fill="#8884d8" animationDuration={2000}/>
                    </BarChart>
                </div>
            </div>
            <div style={{ display: "flex", flexDirection: "column", flex:2 }}> 
                <div style={{border: "5px solid rgba(149, 165, 166, .7)", backgroundColor: 'rgba(149, 165, 166, .5)', borderRadius: '15px', marginTop: '40px', marginBottom: '290px', marginLeft: '40px', marginRight: '40px', display: "flex", flexDirection: 'column', flex: 1}}>
                    ...
                    <div style={{fontSize: '16px', marginLeft: '10px', marginRight: '10px', marginTop: '10px', marginBottom: '10px'}}>The Equus has the longest teeth and cheek size span of its generation, at 32mm and 17.6cm, respectively, which would be useful for grazing long grasses (as grasses are fibrous, and wear down teeth easily, which means that long teeth are required). The Equus does not have a further natural evolution, and evolved from the Merychippus separately from the Pliohippus; however, it has been a subject of selective breeding by humans trying to create the fastest horse possible.</div>
                </div>
                <Link to="/" style={{display: 'flex', flex: 1, marginLeft: '20px', marginBottom: '20px'}}><img src={home} style={{width: 60, height: 60}}/></Link>
            </div>
        </div>
    </div>
  );
